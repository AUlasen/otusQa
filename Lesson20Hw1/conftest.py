...
class DbHandler(logging.Handler):
    ...
    def emit(self, record):
        try:
            ...
            cursor = self.cursor
            # cursor.execute("INSERT INTO logs(msg) VALUES ('".join(msg).join("')"))
            cursor.execute("INSERT INTO logs(msg) VALUES (?)", [msg])
            self.conn.commit()
            self.flush()
        except Exception:
