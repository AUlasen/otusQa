...
from Lesson11_DnD.exceptions import TestErrorException
from Lesson11_DnD.locator import *
from Lesson11_DnD.page import BasePage
...
class CustomMenuPage(BasePage):
    def switch_elements(self, el1_text, el2_text):
        menu: WebElement = self.driver.find_element(*CustomMenuLocators.CUSTOM_MENU_UL)
        el1: WebElement = self.find_item(menu, el1_text)
        el2: WebElement = self.find_item(menu, el2_text)

        #It doesnt work: ActionChains(self.driver).move_to_element(el1).perform()
        self.driver.execute_script("arguments[0].scrollIntoView();", el1)
        time.sleep(2)
        ActionChains(self.driver).drag_and_drop(el1, el2).perform()

    def find_item(self, menu, item_text):
        element_list = menu.find_elements(By.CSS_SELECTOR, "dt.custommenu-item-handle")
        for el in element_list:
            span = el.find_element(By.CSS_SELECTOR, "span.custommenu-item-title")
            span_text = span.text
            if span_text == item_text:
                return el

    def get_location(self, item_text):
        menu: WebElement = self.driver.find_element(*CustomMenuLocators.CUSTOM_MENU_UL)
        el: WebElement = self.find_item(menu, item_text)
        return el.location
